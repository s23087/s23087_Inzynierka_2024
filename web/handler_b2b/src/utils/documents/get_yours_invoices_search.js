...
export default async function getSearchYoursInvoices(
  ...
  if (isOrg) {
    url = `${process.env.API_DEST}/${dbName}/Invoices/get/purchase/org?search=${search}${params.length > 0 ? "&" : ""}${params.join("&")}`;
  } else {
    const userId = await getUserId();
    url = `${process.env.API_DEST}/${dbName}/Invoices/get/purchase/${userId}?search=${search}${params.length > 0 ? "&" : ""}${params.join("&")}`;
  }
  try {
