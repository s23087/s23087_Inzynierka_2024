...
export default async function getSearchSalesInvoices(
  ...
  if (isOrg) {
    url = `${process.env.API_DEST}/${dbName}/Invoices/get/sales/org?search=${search}${params.length > 0 ? "&" : ""}${params.join("&")}`;
  } else {
    const userId = await getUserId();
    url = `${process.env.API_DEST}/${dbName}/Invoices/get/sales/${userId}?search=${search}${params.length > 0 ? "&" : ""}${params.join("&")}`;
  }
  try {
