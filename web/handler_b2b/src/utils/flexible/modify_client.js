...
export default async function modifyClient(orgId, prevState, state, formData) {
  ...
  try {
    ...
    if (info.ok) {
      return await setAvailabilityStatusesToClient(prevState, orgId, dbName, userId);
    }
  ...
  function getData() {
    return {
      orgId: orgId,
      orgName: formData.get("name") === prevState.orgName ? null : formData.get("name"),
      nip: nip === "" ? null : parseInt(nip),
      street: formData.get("street") === prevState.street
        ? null
        : formData.get("street"),
      city: formData.get("city") === prevState.city ? null : formData.get("city"),
      postalCode: formData.get("postal") === prevState.postalCode
        ? null
        : formData.get("postal"),
...
async function setAvailabilityStatusesToClient(prevState, orgId, dbName, userId) {
